...
namespace ModelContertApp
{
    public partial class Main : Form
    {
        ...
        private void buttonSaveFile_Click(object sender, EventArgs e)
        {
            ...
            if (Path.GetExtension(this.textBoxFileName.Text) == ".ifc")
            {
                sfd.Filter = "BIMPlatform Model File (*.bpm)|*.bpm|xBIM File (*.xbim)|*.xbim|RDF File (*.rdf)|*rdf";
            }
            if (Path.GetExtension(this.textBoxFileName.Text) == ".obj")
            {
        ...
        private void buttonConvert_Click(object sender, EventArgs e)
        {
            ...
            if (extension == ".ttl")
            {
                ...
                int buildingLevels = TurtleBOTConverter.GetBuildingLevels(this.textBoxFileName.Text);
                if (this.nameTextBox.Text == null)
                {
                    MessageBox.Show("You need the building name!");
                    return;
                }


                Model dbmsModel = GetDBMSApiModelFromIfc(this.textBoxFileName.Text, scale, flipTriangles);
                string buildingName = this.nameTextBox.Text;
                string siteName = this.siteTxtBox.Text;
                TurtleBOTConverter.buildingName = this.nameTextBox.Text;
                TurtleBOTConverter.siteName = this.siteTxtBox.Text;
                TurtleBOTConverter.TTL_preProcess();    
                TurtleBOTConverter.GenerateBuildingFloors(buildingLevels);
                string rdfString = TurtleBOTConverter.ModelToTurtle(dbmsModel);
                File.WriteAllText(this.textBoxSaveFileLocation.Text, rdfString);
            }
