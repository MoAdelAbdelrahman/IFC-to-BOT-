...
namespace ModelConverter
{
    public static class ObjConverter
    {
        ...
        public static CatalogObject ConvertObjFile(FileStream fileStream, string objectName,double scale, bool flipTriangles, bool flipYZ)
        {
            ...
            LoadResult result = objLoader.Load(lines.ToArray());
        ...
        public static CatalogObject ConvertObjFile(LoadResult objectLoadResult, string objectName, double scale, bool flipTriangles, bool flipYZ)
        {
            ...
            foreach (var group in objectLoadResult.Groups)
            {
                ...
                foreach (Tuple<Vector3D, List<Vector3D>> face in faces)
                {
                    ...
                    try
                    {
                        tempTriangleInts = Utils.EarClip(face.Item2, face.Item1);
                    }
                    catch
                    {
                        continue;
                ...
                Component component = new Component()
                {
                    ...
                    MaterialId = group.Material != null ? group.Material.Name : Material.Default().Name,
        ...
        public static Tuple<Vector3D, List<Vector3D>> GetFace(Face face, LoadResult objectLoadResult, bool flipYZ)
        {
            ...
            {
                int normalIndex = (face[i].NormalIndex + objectLoadResult.Normals.Count) % objectLoadResult.Normals.Count;
                int vertexIndex = (face[i].VertexIndex + objectLoadResult.Vertices.Count) % objectLoadResult.Vertices.Count;
                if (counter > 0 && !NormalsEqual(normal, objectLoadResult.Normals[Math.Abs(normalIndex) - 1]))
                {
                    // weird case when not all verticies have the same normal...
                    fixNormals = true;
                }
                normal = objectLoadResult.Normals[normalIndex - 1];             // Every vertex has a normal?
                Vertex vertex = objectLoadResult.Vertices[vertexIndex - 1];     // Check why its -1?
